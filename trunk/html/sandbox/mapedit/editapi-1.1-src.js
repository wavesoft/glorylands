...
function ui_new() {
	if (ui_changed) {
		if (!window.confirm('This document is changed! Do you really want to clear the map without saving it?')) {
			return;
		}
	}
	ui_changed=false;
...
function ui_load() {
	if (ui_changed) {
		if (!window.confirm('This document is changed! Do you really want to load a new map without saving it?')) {
			return;
		}
	}
...
function undo_start_group() {
	if (in_undo) return;
	undo_group_started=true;
	undo_active=[];
}

function undo_finish_group() {
	if (in_undo) return;
	if (!undo_group_started) return;
	undo_group_started=false;
	undo_cache.push(undo_active);
	if (undo_cache.length > 20) {
		undo_cache.shift();
	}
}

function undo_store(fn_undo, data) {
	if (in_undo) return;
	if (!undo_group_started) return;
	undo_active.push([fn_undo, data]);

	// If we have an undo trigger, it means
	// something is changed. Notify the UI also
	ui_changed=true;
}

function undo_stepback() {
	if (undo_cache.length == 0) return;
	in_undo=true;
	var chunk = undo_cache.pop();
	try {
		for (var i=0; i<chunk.length; i++) {
			chunk[i][0](chunk[i][1]);
		}
	} catch(e) {
		window.alert('Undo error'+e);
	}
	in_undo=false;
}
...
function paint_getblock(x,y,layer) {
	var id=x+','+y;
	if ($defined(paint_grid[layer-1][id])) {
		var src=paint_grid[layer-1][id].src;
		var parts=src.split('images/tiles/');
		return parts[1];
	} else {
		return false;
	}
...
function paint_clear(sx,sy,layer) {
	var x = sx+scroll_offset.x;
	var y = sy+scroll_offset.y;
	if (!layer) layer=paint_layer;

	undo_store(function(data) {
		if (data[2] == false) {
		} else {
			paint_put(data[0], data[1], data[2], data[3]);
		}
	}, [x,y,paint_getblock(x,y,layer),layer]);
	
	var id=x+','+y;
	if ($defined(paint_grid[layer-1][id])) {
		paint_grid[layer-1][id].remove();
		paint_grid[layer-1][id]=null;
	}	
}

function paint_put(sx,sy,image, layer) {
	var x = sx+scroll_offset.x;
	var y = sy+scroll_offset.y;
	if (!layer) layer=paint_layer;
	...
	undo_store(function(data) {
		if (!data[2]) {
			paint_clear(data[0],data[1],data[3]);
		} else {
			paint_updateblock(data[0], data[1], data[2], data[3]);
		}
	}, [x,y,paint_getblock(x,y,layer),layer]);
	
	var im = paint_updateblock(x,y,image,layer);
	im.setStyles({
		'left': x*32,
		'top': y*32
    });
...
function cgrid_draw(x,y,value) {
	...
	undo_store(function(data) {
		if (!data[2]) {
			cgrid_clear(data[0],data[1]);
		} else {
			cgrid_draw(data[0],data[1],data[2]);
		}
	}, [x,y,cgrid_datagrid[y][x]]);
...
$(window).addEvent('load', function(e){	
	...
	$(window).addEvent('beforeunload', function(e) {
		if (ui_changed) {
			return "You have unsaved changes! You will loose them if you leave this site!";
		}
	});
