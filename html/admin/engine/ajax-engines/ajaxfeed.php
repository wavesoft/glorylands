...
while ($row = $sql->fetch_array()) {
	// Echo cell data
	$gridData[$row['x']][$row['y']]['layers'] = array($row['layer0'],$row['layer1'],$row['layer2']);
	//$res_str .= "|".($row['x']-$basex).",".($row['y']-$basey).",".$row['layer0'].",".$row['layer1'].",".$row['layer2'].",".$unitInfo;
}

// Load SQL unit info
$ans = $sql->query("SELECT `UID`, `typeName`, `pos_x`, `pos_y`, `map_img`, `map_level` FROM `units_instances` WHERE `pos_map` = '".$map."' AND `pos_x` >= '$basex' AND `pos_x` <= '$maxx' AND `pos_y` >= '$basey' AND `pos_y` <= '$maxy'");
if (!$ans) {
	$res_num = -1;
	$res_str = "MySQL Error: ".$sql->getError();
}
while ($row = $sql->fetch_array()) {
	if (isset($gridData[$row['pos_x']][$row['pos_y']]['units'])) {
		...
		if (!isset($gridData[$row['pos_x']][$row['pos_y']]['layers'])) {
			$gridData[$row['pos_x']][$row['pos_y']]['layers'] = array('','','');
		}
		if ($row['map_img']!=='') {
			$gridData[$row['pos_x']][$row['pos_y']]['layers'][$row['map_level']] = $row['map_img'];
		}
	}
}
...
foreach ($gridData as $x => $gridData_x) {
	foreach ($gridData_x as $y => $info) {
		...
		$res_str .= "|".($x-$basex).",".($y-$basey).",".$info['layers'][0].",".$info['layers'][1].",".$info['layers'][2].",".$unitInfo;
	}
}
