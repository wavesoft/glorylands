...
<title>Manage package installation files</title>
...
if ($_REQUEST['a'] == 'upload') {
	...
	$sql->query("INSERT INTO `system_packages_install` (`package`, `imode`, `use`, `data`) VALUES
		({$pid}, '{$type}', 'INSTALL', '".mysql_escape_string($_FILES['uploadfile']['name'])."')
	");
	$index=$sql->query_and_get_value("SELECT `index` FROM `system_packages_install` ORDER BY `index` DESC LIMIT 0,1");
...
if ($_REQUEST['a'] == 'upload_data') {
	...
	");
		...
		foreach ($_REQUEST['f_name'] as $index => $filename) {
			...
			// Update SQL entry
			$sql->editRow('system_packages_install', "`index` = $index", array(
				'imode' => $_REQUEST['f_mode'][$index],
		...
		<center>
		<div class="centerblock" align="center">
		<p><b><?php echo $changed; ?></b> installation files are updated on the package <b><?php echo $row['name']; ?></b></p>
		...
		foreach ($_REQUEST['file'] as $index => $ack) {
			...
			$ans = $sql->query("SELECT * FROM `system_packages_install` WHERE `index` = $index");
			$info = $sql->fetch_array();
			
			@unlink($scripts.'/'.$info['data']);

			$ans = $sql->query("DELETE FROM `system_packages_install` WHERE `index` = $index");		
			$changed++;
		}
		...
		<center>
		<div class="centerblock" align="center">
		<p><b><?php echo $changed; ?></b> installation files are deleted on the package <b><?php echo $row['name']; ?></b></p>
		<p><a href="packagemanifest.php?guid=<?php echo $guid; ?>">Click here to go back to the manifest management</a></p>
		</div>
		</center>
		<?php
...
<table width="100%">
<tr>
	<td width="35" rowspan="2" valign="top"><img src="../images/edit_add32.gif" align="absmiddle" /></td>
	<td align="left"><font size="+2"><b> <?php echo $row['name']; ?></b></font></td>
</tr>
<tr>
	<td align="left">Create new installation script or SQL file</td>
</tr>
<tr>
	<td colspan="2">
...
?>	
	...
	<table style="width: 100%;">
		...
		<tr>
			...
			<option value="SCRIPT" <?php if ($info['imode']=='SCRIPT') echo 'selected="selected"'; ?>>PHP Script</option>
			<option value="SQL" <?php if ($info['imode']=='SQL') echo 'selected="selected"'; ?>>SQL Queries</option>
			</select>
			</td>
		</tr>
		<tr>
			...
			<option value="INSTALL" <?php if ($info['use']=='INSTALL') echo 'selected="selected"'; ?>>Installation</option>
			<option value="ENABLE" <?php if ($info['use']=='ENABLE') echo 'selected="selected"'; ?>>Enabling</option>
			</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			<td>&nbsp;</td>
			<td>
			<input type="button" value="&uArr; Table Structure" onclick="winpopup('popup_tablestruct.php');" />
			<input type="button" value="&uArr; Table Data" onclick="winpopup('popup_tabledata.php');" />
			</td>
		</tr>
		<?php
		}
...
<?php
		...
		<center>
		<div class="centerblock" align="center">
		<p>New installation file <b><?php echo $_REQUEST['f_name']; ?></b> is added on the package <b><?php echo $row['name']; ?></b></p>
		<p><a href="packagemanifest.php?guid=<?php echo $guid; ?>">Click here to go back to the manifest management</a></p>
		</div>
		</center>
		<?php
...
<tr>
	...
	<table style="width: 100%;">
		...
		<tr>
			...
			<option value="ENABLE">Enabling</option>
			</select>
			</td>
		</tr>
		<tr>
